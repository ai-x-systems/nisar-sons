...
export default function ProductApplications() {
  return (
    <section aria-labelledby="applications-heading" className="py-20 bg-fog">
      <div className="container">
        <p className="eyebrow">Where It&apos;s Used</p>
        <h2 id="applications-heading" className="font-display text-2xl sm:text-[1.75rem] font-semibold text-charcoal mt-3 leading-tight tracking-tight max-w-xl">
          Applications
        </h2>
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mt-10 list-none">
          {APPLICATIONS.map((app, i) => (
            ...
            >
              <span className="flex items-center justify-center w-11 h-11 rounded-lg bg-brand/10 text-brand-dark">
                <app.icon className="w-5 h-5" strokeWidth={1.75} aria-hidden="true" />
              </span>
              <h3 className="font-display text-[15.5px] font-semibold text-charcoal mt-5">{app.title}</h3>
              <p className="text-[13px] text-graphite-light mt-1.5 leading-relaxed">{app.desc}</p>
            </motion.li>
          ))}
        </ul>
      </div>
