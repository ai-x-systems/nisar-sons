    ...
    <section id="about" aria-labelledby="about-heading" className="py-24 bg-white">
      <div className="container grid lg:grid-cols-2 gap-16 items-center">
        ...
        >
          ...
          <h2 id="about-heading" className="font-display text-3xl sm:text-[2.35rem] font-semibold text-charcoal mt-3 leading-tight tracking-tight">
        ...
        <motion.dl
          ...
          className="grid grid-cols-2 gap-px bg-line border border-line rounded-xl overflow-hidden m-0"
        >
          {STATS.map((s) => (
            <div key={s.label} className="bg-fog p-8">
              <dd className="font-display text-3xl sm:text-4xl font-semibold text-charcoal tracking-tight m-0">{s.value}</dd>
              <dt className="font-mono text-[11px] tracking-widest2 uppercase text-graphite-light mt-2 font-normal">{s.label}</dt>
            </div>
          ))}
        </motion.dl>
      </div>
